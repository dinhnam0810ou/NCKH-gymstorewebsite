...
public class ShipperRepositoryImpl implements ShipperRepository {
    ...
    public List<Object[]> revenue(int quarter, int year) {
        ...
        q.multiselect(rS.get("id"), rS.get("firstname"), rS.get("lastname"), b.sum(b.prod(rA.get("price"),
                rP.get("discount"))));
        q.groupBy(rA.get("shipperId"));
    ...
    @Override
    public int countRating(int shipperId) {
        Session session = this.sessionFactory.getObject().getCurrentSession();
        Query q = session.createQuery("SELECT Sum(rating) FROM Comment Where shipperId.id=:id");
        q.setParameter("id", shipperId);

        try {
            if (q.getSingleResult().toString() != null) {
                return Integer.parseInt(q.getSingleResult().toString());
            }
        } catch (Exception e) {
            return 0;
